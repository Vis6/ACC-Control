...
if rl_mode == 'train':
	...
	# car 2 reinforcement learning
	w.add(c2)
	c2_rl = RLSteering(c2, w, episode=episode_num)
	c2_rl.q_learning()
	c2_rl.delete_vars()
	save_rl_policy(c2_rl, 'car2_rl.pkl')
elif rl_mode == 'test':
	w.add(c1)
	w.add(c2)
	...
	c2_rl = RLSteering(c2, w)
	saved_rl_obj = read_rl_policy('car2_rl.pkl')
	c2_rl.overwrite(saved_rl_obj)

	# simulate
	# car 1 - position reset
	# c1.center.x = 91.75
	# c1.center.y = 60
	# c1.heading = np.pi / 2
	# c1.center.x = c1_rl.car_x_init
	# c1.center.y = c1_rl.car_y_init
	# c1.heading = c1_rl.car_heading_init

	# car 2 - position reset
	# c2.center.x = 60
	# c2.center.y = 95.25
	# c2.heading = np.pi
	# c2.center.x = c2_rl.car_x_init
	# c2.center.y = c2_rl.car_y_init
	# c2.heading = c2_rl.car_heading_init
	...
	for k in range(600):
		...
		heading_new = c2.heading + c2_rl.get_rl_action(c2.center.x, c2.center.y)
		c2.set_control(heading_new, 0.06)  # no acceleration
